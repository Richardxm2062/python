...
def solve(n, row, col) :
    ...
    while True :
        #首行顺序填充
        for i in range(0+ct, col+ct, 1):
            out_mat[ct][i] =  out_list[0]
            out_list.pop(0) 
        ...
        #中行升序填充 
        for ele in out_mat[1+ct : (row-1)+ct] :
            ele[-1 - ct] = out_list[0]
            out_list.pop(0)
            p = permission(out_list)
            if p == 0 :         #填充完毕 立刻停止
        ...
        #尾行逆序填充 
        for i in range((col-1)+ct, -1 + ct, -1) :
            out_mat[-1 - ct][i] = out_list[0]
            out_list.pop(0)
            p = permission(out_list)
            if p == 0 :         #填充完毕 立刻停止
                return out_mat
        ...
        #中行降序填充
        for ele in out_mat[(row-2)+ct :0+ct : -1] :
            ele[ct] = out_list[0]
            out_list.pop(0)
            p = permission(out_list)
            if p == 0 :         #填充完毕 立刻停止
