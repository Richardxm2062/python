...
考虑放入第i个物品的情况
I. w[i]>j 即物品大于当前j背包容量 那么 dp[i][j] = dp[i-1][j]    (继承前i-1个物品所获得的最大价值)

II. w[i]<j 即可以选择放入背包
    1.放入 : dp[i][j] = dp[i-1][j-w[i]] + v[i]  (部分继承放入前i-1个物品且容量为 j-w[i]的dp值)
...
def dynamic_solve(ct,num,wg,val) :
    ...
    #从前往后遍历进行填表,(为了防止i-1的边界问题创建二维数组的时候均高了一维)
...
def violent_solve(ct,num,wg,val):
    ...
    #当前物品小于背包容量
    no = violent_solve(ct,num-1,wg,val)     #不放入
    yes = violent_solve(ct - wg[num-1],num-1,wg,val) + val[num-1]   #放入
    
    return max(yes,no)      #返回最大的一个值
